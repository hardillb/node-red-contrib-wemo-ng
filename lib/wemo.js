		...
		request.get(location.href, function(err, res, xml) {
			xml2js.parseString(xml, function(err, json) {
				...
				if (device.deviceType == "urn:Belkin:device:bridge:1") {
									...
									xml2js.parseString(list, function(err, result2) {
										if (!err) {
											...
											for (var i=0; i<devinfo.length; i++) {
												...
												if (!wemo.devices[device.serialNumber + "-" + light.id]){
													wemo.devices[device.serialNumber + "-" + light.id] = light;
													wemo.emit('discovered', device.serialNumber + "-" + light.id);
												} else {
													wemo.devices[device.serialNumber + "-" + light.id] = light;
												}
											}
											...
											if (groupinfo) {
												...
												if (!wemo.devices[device.serialNumber + "-" + group.id]) {
													wemo.devices[device.serialNumber + "-" + group.id] = group;
													wemo.emit('discovered', device.serialNumber + "-" + group.id);
												} else {
													wemo.devices[device.serialNumber + "-" + group.id] = group;
												}
											}
					...
					if (!wemo.devices[device.serialNumber]) {
						wemo.devices[device.serialNumber] = socket;
						wemo.emit('discovered',device.serialNumber);
					} else {
						wemo.devices[device.serialNumber] = socket;
					}
				}
